function [Beta,F,dim]=cca_modeling(X,Y,mode,var_limit)
...
if (mode==0)
   ...
   if (h<2)|(press_total<press_total_last)
      [A,B,V]=cca(X,Y,h);
      if (nargout==5), dim=h;end
   else
      [A,B,V]=cca(X,Y,h-1);
      if (nargout==5), dim=h-1;end
   end
   
   %Beta is an approximation of the regression coefficients
   Beta=(pinv(B')*V.^0.5*A')';
   ...
   diag_V=diag(V);
   for dd=1:m2
      contribution_rate=sum(diag_V(1:dd))/sum(diag_V);
